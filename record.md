...
## 19. 추가 수정 (v0.2.3 자이로 실시간 Z축 적분 시 틸트 보정 투영 적용)
 
### 19.1 수정 이유
수평 회전판의 물리적 기울어짐 및 센서 하우징 마운팅 요동 오일러각으로 인해, 센서 Z축 자이로(gyro_raw[2])를 그대로 누적할 경우 순수 지구 Z축(NED Yaw) 방향의 회전 각속도가 온전히 투영되지 않고 왜곡되어 적분각에 심각한 스케일 수축이나 오차가 발생하는 문제를 해결하기 위함.
 
### 19.2 수정 계획 및 예상 결과
- 위치: verification_tool/test_phase2_3_mag_cal.py
- 내용: gyro_bias를 3축 전체 벡터로 로드하여 3축 자이로 데이터(gyro_raw - gyro_bias)에서 바이어스를 소거한 뒤, 최초 0도 안착 시 도출하여 고정한 판 기울어짐 정합 행렬 R_tilt_fixed 를 곱해 지구 수직축(Z) 각속도 성분을 정확하게 투영 추출하여 적분함.
- 예상 결과: 기하학적 틸트 및 마운팅 꼬임 오차가 완전히 제거되어, 수평 360도 회전 시 자이로 적분 궤적이 [0.0, 90.0, 180.0, 270.0, 360.0] 부근으로 완벽히 일치하여 정상 수렴함.
 
### 19.3 수정 내용
- test_phase2_3_mag_cal.py 내 3축 gyro_bias 로드 및 실시간 자이로 적분 루프 내 R_tilt_fixed 행렬 곱 투영 식 이식.
...
## 20. 추가 수정 (v0.2.3 자이로 실시간 바이어스 즉석 영점 보정 적용)

### 20.1 수정 이유
외부 캘리브레이션 파라미터 파일(gyro_params.npz) 로드 의존성을 제거함에 따라, 실시간 데이터 수집 시작 시점인 0도 정지 상태에서의 100샘플 자이로 평균값을 이용하여 즉시 실시간 자이로 바이어스(gyro_bias)를 갱신 및 보정 적용함으로써 누적 적분 드리프트를 원천 억제하기 위함입니다.

### 20.2 수정 계획 및 예상 결과
- 위치: verification_tool/test_phase2_3_mag_cal.py
- 내용: 사용자가 0도 첫 거치 완료 후 엔터/스페이스 키를 눌러 idx == 0 루프를 탈출한 직후에 gyro_window의 100샘플 평균을 계산하여 gyro_bias에 대입하고, 터미널에 갱신 로그를 출력하도록 이식합니다.
- 예상 결과: 0도 안착 완료 엔터 시점에 gyro_bias가 실시간으로 자동 갱신되며, 이후 회전 및 거치 동작 단계(idx > 0)부터 바이어스가 소거된 깨끗한 각속도가 R_tilt_fixed 투영 행렬을 거쳐 적분되므로 드리프트 오차가 극도로 억제됩니다.

### 20.3 수정 내용
- test_phase2_3_mag_cal.py 내 0도 안착 완료(R_tilt_fixed가 None인 최초 1회) 분기 진입 시 `gyro_bias = np.mean(gyro_window, axis=0)` 계산 및 로깅 구문을 추가하였습니다.

### 20.4 실제 결과
- 코드 이식 완료 및 정적 구문 오류 없음을 확인하였습니다. (실제 궤적 수렴 여부는 사용자의 로컬 쉘 실행을 통해 최종 검증)

---

## 21. 추가 수정 (v0.2.3 자이로 17.5배 스케일 오차 교정을 위한 펌웨어 수정)

### 21.1 수정 이유
SparkFun ISM330DHCX 라이브러리의 gyroData 데이터가 이미 dps에 1000이 곱해진 mdps(milli-degrees per second) 단위로 반환되는 스펙임에도, LSB 스케일 계수인 0.0175를 오곱하여 자이로 측정값이 실제보다 17.5배 부풀려진 채 송출되던 현상을 교정하기 위함입니다.

### 21.2 수정 계획 및 예상 결과
- 위치: firmware/raw/raw.ino 및 firmware/calibrated/calibrated.ino
- 내용: `(gyroData.xData * 0.0175) * (PI / 180.0)` 부분을 `(gyroData.xData / 1000.0) * (PI / 180.0)` 으로 수정합니다.
- 예상 결과: 아두이노가 PC로 송출하는 자이로 값의 물리 단위가 실제 rad/s로 올바르게 수렴하게 되며, 이에 따라 test_phase2_3_mag_cal.py 실행 시의 누적 자이로 적분 궤적 오차가 제거되어 360도 회전 시 360도 부근으로 정상 복원됩니다.

### 21.3 수정 내용
- raw.ino 및 calibrated.ino 내 자이로 변환부의 환산 계수를 `/ 1000.0`으로 수정 적용하였습니다.

### 21.4 실제 결과
- 펌웨어 코드의 자이로 감도 스케일 계수 수정 완료. (실제 궤적 수렴 여부는 사용자의 로컬 쉘 실행을 통해 최종 검증)

---

## 22. 추가 수정 (v0.3.0 지자기 환경 매핑 폐기 및 인천 고정 복각 레퍼런스 수술)

### 22.1 수정 이유
신뢰도가 떨어지는 실내 지자기 매핑 데이터(env_params.npz) 로딩에 자세 추정이 왜곡되는 문제를 차단하기 위함입니다.

### 22.2 수정 계획 및 예상 결과
- static_initialization.py 내의 env_params.npz 로드 로직을 제거하고 인천 표준 복각 54.3도 절대 레퍼런스([0.583503, 0.0, 0.812108])를 고정 대입합니다.
- 결과: 자북 벡터의 기준 신뢰도가 고정되어 3p/6p/9p 보정 파라미터 간의 수치 성능 교차 비교가 가능해집니다.

### 22.3 수정 내용
- static_initialization.py 수정 및 인천 고정 복각 적용 완료.
- mag_environment_mapping.py 파일 삭제 완료.

### 22.4 실제 결과
인천 지자기 표준 데이터 정합 확인 완료 (Commit: `541c561`).

---

## 23. 추가 수정 (v0.3.0 피팅 솔버 모듈화 및 imu_core/math.py 이관)

### 23.1 수정 이유
각기 분산되어 하드코딩되어 있던 가속도 12-param, 자력계 3/6/9-param 및 자이로 바이어스 솔버를 하나의 모듈로 정합하기 위함입니다.

### 23.2 수정 계획 및 예상 결과
- imu_core/math.py 내에 전역 캘리브레이션 수학 알고리즘을 이식하고 외부 호출 구조로 리팩토링합니다.

### 23.3 수정 내용
- math.py 모듈로 피팅 알고리즘 일괄 이관 완료.

### 23.4 실제 결과
모듈 호출로 데이터 처리가 DRY 원칙을 충족하게 됨 (Commit: `06d2d06`).

---

## 24. 추가 수정 (v0.3.0 9축 통합 보정 연쇄 구동기 신설 및 C++ 헤더 컴파일러 구축)

### 24.1 수정 이유
Ellipsoid 보정 초기화 후 Task-Aware 최종 보정 및 calibrated.ino 헤더 이식을 자동화하기 위함입니다.

### 24.2 수정 계획 및 예상 결과
- calibration_tool/integrated_calibration.py를 신설하여 1~3단계(가속도->자이로->자력) 연쇄 구동 및 'calib_params.h' C++ 컴파일러 이식을 자동화합니다.

### 24.3 수정 내용
- integrated_calibration.py 작성 및 펌웨어 경로 자동 덮어쓰기 로직 완료.

### 24.4 실제 결과
보정 후 즉시 펌웨어 컴파일이 가능한 이식 툴체인 가동 성공 (Commit: `c7af61e`).

---

## 25. 추가 수정 (v0.3.0 Task-Aware 솔버 SVD 예외 크래시 디버깅)

### 25.1 수정 이유
합성 더미 데이터 인가 테스트 시, 최적화 변수 발산에 따른 SVD Convergence Fail 예외 중단 현상을 해결하기 위함입니다.

### 25.2 수정 계획 및 예상 결과
- W 행렬의 exp 발산 임계치 제한(np.clip) 및 NaN/inf 유입 차단 페널티 인가를 imu_core/math.py에 적용합니다.

### 25.3 수정 내용
- math.py 내 calibrate_mag_task_aware 함수 clip 필터링 및 예외 우회 코드 보강.

### 25.4 실제 결과
최종 3p/6p/9p 최적화 솔버 연산 루프가 중단 없이 가동 수렴 성공 (Commit: `8bf0585`).
