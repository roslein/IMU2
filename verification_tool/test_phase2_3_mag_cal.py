            ...
            while True:
                ...
                while len(byte_buffer) >= PACKET_SIZE:
                    ...
                    if idx > 0:
                        ...
                        # [주의 및 아키텍처 주석]
                        # 1. 이 코드에서 부호 반전(-gz_cal)을 수행하는 이유:
                        #    R_tilt_fixed가 downward [0,0,-1] 기준이므로 투영된 지구 Z축(ENU 기준 Upward)에 대해 오른손 법칙 상 
                        #    시계 방향 회전은 물리적으로 음(-)의 각속도가 됩니다. 반면 자력계 2D 평면 Yaw(atan2)는 시계 방향 회전 시 
                        #    양(+)으로 증가하여 부호가 반대입니다. 1대1 대조 및 RMSE 연산을 위해 자이로 부호를 반전하여 누적합니다.
                        # 2. 차기 3D 자세 융합 필터(EKF/Madgwick 등) 코드에서 Z축 부호를 수정하면 안 되는 이유:
                        #    3D EKF는 3축 원시 데이터가 오른손 법칙 좌표계를 충실히 따르고 있어야 쿼터니언 미분 및 보정 연산 기하가 
                        #    완벽히 작동합니다. 특정 축만 인위적으로 부호를 반전하면 3D 좌표계가 왼손계로 일그러져 필터가 발산합니다.
