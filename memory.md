...
*   **ID**: CONCEPT-IMU-04
*   **유형**: [깨달음]
*   **내용**: calibrated.ino 펌웨어의 가속도/자력 물리 스케일(1.0 부근)과 Python 모니터링 스크립트(test_phase1.py)의 출력 포맷 소수점 자릿수(.1f) 간 불정합으로 데이터 노이즈 소멸 착시 현상 발생 가능. 정밀 모니터링 시 출력 해상도를 최소 소수점 4자리(.4f) 이상으로 확장 설계해야 함.

*   **ID**: PATTERN-IMU-02
*   **유형**: [패턴]
*   **내용**: 개별 캘리브레이션 툴(acc, gyro, mag)은 각 센서의 파라미터 백업 파일(.npz) 저장만 분리 담당하고, 펌웨어 최종 이식용 C++ 헤더(calib_params.h)는 독립된 통합 빌더(generate_calib_params.py)에서 일괄 수집/빌드하는 아키텍처 패턴 확립.

*   **ID**: PATTERN-IMU-03
*   **유형**: [패턴]
*   **내용**: 자이로스코프의 3D 스케일 보정 행렬이 미확보된 상태에서 보정 연산을 선제 이식할 때, 보정 행렬 GYRO_W[3][3]를 Identity(단위 행렬)로 채워 펌웨어에 제공하면 데이터 왜곡 없이 오프셋(GYRO_B) 보정 연산을 완벽 적용 가능함.
